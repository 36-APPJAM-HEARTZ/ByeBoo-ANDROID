...
constructor(
    ...
    private val _uiState = MutableStateFlow(
        QuestUiState(
            selectedTab = savedStateHandle.get<String>("selectedTab")
    ...
    fun onDateChange(newDate: LocalDate) {
        ...
        _uiState.update { state ->
            state.copy(
                commonJourneyState = CommonJourneyState(
    ...
    private suspend fun fetchCommonQuests(date: LocalDate) {
        runCatching {
            ...
            val uiAnswers = domainModel.answers
                .map { answer ->
                    CommonAnswerModel(
    ...
    fun loadNextPage() {
        ...
        if (currentState.isLoading || currentState.isPaginationLoading || !currentState.hasNext || currentState.nextCursor == null) return
        ...
        paginationJob = viewModelScope.launch {
            ...
            runCatching {
                ...
                val moreAnswers = domainModel.answers.map { answer ->
                    CommonAnswerModel(
                        answerId = answer.answerId,
                        writer = answer.writer,
                        profileIconRes = mapper.mapToIconRes(answer.profileIcon),
    ...
    private fun updateTimerLockedMinutes(minutes: Long) {
        _uiState.update { state ->
            val updatedGroups = state.myJourneyState.questGroups
                .map { group ->
                    group.copy(
                        quests = group.quests
                            .map { quest ->
    ...
    fun onQuestClicked(questId: Long) {
        viewModelScope.launch {
            val quest = uiState.value.myJourneyState.questGroups
                .flatMap { it.quests }
